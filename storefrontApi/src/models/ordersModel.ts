...
class orderStore {
  ...
  async show(user_id:Number):Promise<Order>{
   ...
   return result.rows[0];
    ...
    const sql=`insert into orders(totalPrice,activeStatus,users_id) values($1,$2,$3) returning *`;

    const result =await connection.query(sql,[order.totalPrice,order.activeStatus,order.user_id]);
